...
What we've learned about how Plex and Jellyfin expect movie libraries to
be organised, and the translation rules that follow from the asymmetries
between the two. This document is the source-of-truth for *what* the
tool translates and *why* the mapping looks the way it does. For *how*
the tool is built, see [DEV.md](./DEV.md).
...
- `{}` content holds key-value metadata: `{imdb-tt...}`, `{tmdb-...}`,
  `{tvdb-...}`, `{edition-Name}`.
- **Plex ignores `[bracketed]` text in filenames entirely.** This is
  ...
  the translation layer relies on — see "Asymmetry of translation" below.
...
name differs in a way that can bite you.
...
- **Casing is convention, not certainty.** Neither doc page
  explicitly states case-sensitivity rules. There's empirical
...
specific labels that get emitted, and the choice is deliberate.
...
outweighs the consistency win of more accurate terminology, so the
...
not to switch is that existing user libraries already use the
...
- **Plex Editions need Plex Pass + Movie Agent v1.28.1+** — but this
  doesn't constrain the translator; it just writes the filenames.
