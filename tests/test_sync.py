...
def test_determine_library_type_detects_plex(tmp_path: Path) -> None:
    movie_dir = tmp_path / "First (1984) {imdb-tt001}"
    movie_dir.mkdir()
    _touch(movie_dir / "First (1984) {imdb-tt001}.mkv", b"v")

    assert determine_library_type(tmp_path) is jp.PlexLibrary


def test_determine_library_type_detects_jellyfin(tmp_path: Path) -> None:
    movie_dir = tmp_path / "First (1984) [imdbid-tt001]"
    movie_dir.mkdir()
    _touch(movie_dir / "First (1984) [imdbid-tt001].mkv", b"v")

    assert determine_library_type(tmp_path) is jp.JellyfinLibrary


def test_determine_library_type_detects_plex_edition(tmp_path: Path) -> None:
...
def test_determine_library_type_returns_none_when_unclear(tmp_path: Path) -> None:
    movie_dir = tmp_path / "Plain Movie (2001)"
    movie_dir.mkdir()
    _touch(movie_dir / "Plain Movie (2001).mkv", b"v")

    assert determine_library_type(tmp_path) is None
